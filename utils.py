...
def gaussian_2d(kernel_size, sigma=1, mean=0):
        ...
        g = normal * np.exp(-((dst-mean)**2 / (2.0 * sigma**2))) 
        # 1st order derivatives
        gx = -x / (2 * np.pi * sigma**4) * np.exp(-((dst-mean)**2 / (2.0 * sigma**2))) 
        gy = -y / (2 * np.pi * sigma**4) * np.exp(-((dst-mean)**2 / (2.0 * sigma**2))) 
